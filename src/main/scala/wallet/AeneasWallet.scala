...
import commons.{PublicKey25519NoncedBox, PublicKey25519NoncedBoxSerializer, SimpleBoxTransaction}
...
case class AeneasWallet(seed: ByteStr, store: LSMStore)
...
object AeneasWallet {
   ...
   def readOrGenerate(settings: ScorexSettings): AeneasWallet = {
      readOrGenerate(settings, settings.wallet.seed)
   }

   def readOrGenerate(settings: ScorexSettings, seed: ByteStr, accounts: Int): AeneasWallet =
      (1 to accounts).foldLeft(readOrGenerate(settings, seed)) { case (w, _) =>
         w.generateNewSecret()
      }

   def readOrGenerate(settings: ScorexSettings, accounts: Int): AeneasWallet =
      (1 to accounts).foldLeft(readOrGenerate(settings)) { case (w, _) =>
         w.generateNewSecret()
      }

   //wallet with applied initialBlocks
   def genesisWallet(settings: ScorexSettings, initialBlocks: Seq[AeneasBlock]): AeneasWallet = {
